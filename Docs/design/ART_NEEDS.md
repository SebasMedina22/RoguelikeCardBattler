...
| N2 | Caras de carta del draft | `NewRunController.BuildDraftColumn` (usa `CardDefinition` de `draftFaces`) | render de arte cableado (mismo criterio que C7); fallback a texto sin arte | igual que C7 (512×768 vertical, PNG con alpha) | ✅ **Drop-in** — **desbloqueado por C7** (campo `Art` en `CardDefinition`): asignar el sprite a la cara → el botón del draft muestra la imagen | P2 | `placeholder-IA` |
